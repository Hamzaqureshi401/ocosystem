...
<div class="row"  style="margin-bottom:0;">
	...
	<?php echo $__env->make('data.databuttons', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
</div>
	...
	<div class="modal-body"
		...
		<div class="row">
			<div class="b-col" style="margin-left:0;" >
				<button href="<?php echo e(url('/documentdealer')); ?>"
...
<script>
	var merchantLinkId = '';
	var ownerUserId = '<?php echo e($user_id); ?>';
    ...
    var dealerTableRow;
    var companyId;
    var locationipaddr='<?php echo e($locationipaddr); ?>';
	...
	var dealerDataTable = $('#dealer_tbl').DataTable({
		...
		autoWidth: false,
		bFilter: true,
		ajax: "<?php echo e(route('data.ajax.getDealerData' )); ?>",
		...
		columns: [
			...
			},
			{
				...
				mRender: function(data, type, full) {
					...
					template = template.replace("<image>",'<img style="width:25px;height:25px;cursor:not-allowed; filter: grayscale(100%) brightness(160%);" class="mt=0 mb-0 text-center"src=\"<?php echo e(asset("/images/bluecrab_25x25.png")); ?>\"/>')
				} else {
					...
					template = template.replace("<image>",'<img style="width:25px;height:25px;cursor:pointer" class="mt=0 mb-0 text-center"src=\"<?php echo e(asset("/images/bluecrab_25x25.png")); ?>\"/>')
				}
					return template;
				}
			},
				...
				mRender: function(data, type, full) {
					...
					if (full.owner_user_id != ownerUserId) {
						if (full.delete_status == 'active') {
							...
							template = template.replace("<image>",'<img style="width:25px;height:25px;cursor:pointer" class="mt=0 mb-0 text-center"src=\"<?php echo e(asset("/images/redcrab_25x25.png")); ?>\"/>')
							...
							template = template.replace("<image>",'<img style="width:25px;height:25px;cursor:pointer" class="mt=0 mb-0 text-center"src=\"<?php echo e(asset("/images/redcrab_disabled_25x25.png")); ?>\"/>')
						}
					}
					return template;
				}
	...
	$("#dealer_tbl").on('click', ".js-blue-crab-modal", function() {
		...
		var own_merchant_id = <?php echo e($globalAuth->get_data('merchant_id')); ?>;
	...
	$('.creditLimitModal').on('hidden.bs.modal', function (e) {
		$.ajax({
			url:"<?php echo e(route('data.ajax.saveMerchantCreditLimit')); ?>",
			data: {
    ...
    {
        $.ajax({
            url:'<?php echo e(route('data.ajax.changeOnewayRelationStatus')); ?>',
            type: 'POST',
            data: {
    ...
    function owApproveLinking() {
        $.ajax({
            url:'<?php echo e(route('data.ajax.saveOnewayRelation')); ?>',
            type: 'POST',
            data: {
                companyId: companyId,
                ptype: 'dealer',
	...
	function approveLinking() {
		$.ajax({
			url:'<?php echo e(route('data.ajax.saveMerchantLinkRelation')); ?>',
			type: 'POST',
			data: {
	...
	{
		$.ajax({
			url:'<?php echo e(route('data.ajax.deactivateMerchantLinkRelation')); ?>',
			type: 'POST',
			data: {
				merchantLinkRelationId: merchantLinkRelationId,
				status: 'inactive'
	...
	{
		$.ajax({
			url:'<?php echo e(route('data.ajax.activateMerchantLinkRelation')); ?>',
			type: 'POST',
			data: {
				merchantLinkRelationId: merchantLinkRelationId,
				merchantLinkId: merchantLinkId,
	...
	function deleteLinking() {
		if (merchantId != null) {
			$.ajax({
				url:'<?php echo e(route('data.ajax.delMerchantTwoWayLinking')); ?>',
				type: 'POST',
				data: {
					merchantLinkId: merchantLinkId,
			...
			$.ajax({
				url:'<?php echo e(route('data.ajax.delMerchantOnyWay')); ?>',
				type: 'POST',
				data: {
					companyId: companyId
				},
	...
	function getdataMerchantID(getID) {
		var route = "<?php echo e(url('/tracking-report')); ?>/" + getID;
	...
	function getmerid (id){
			...
			$.ajax({
			url:'<?php echo e(route('get.mer.id')); ?>',
			type: 'GET',
			data: {
				pass_id: id,
			},
...
<?php /**PATH E:\ocosystem\resources\views/data/dealer.blade.php ENDPATH**/ ?>