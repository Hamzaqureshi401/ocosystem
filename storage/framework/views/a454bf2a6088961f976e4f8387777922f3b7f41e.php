		...
		<div class="modal-content bg-greenmidlobster"
			...
			<div class="row">
				...
				<h5 style="cursor: pointer;"
					onclick="add_terminal('<?php echo e($location[$count]->id); ?>')">
					<?php echo e($location[$count]->branch); ?>
...
<script>
    ...
    $(document).ready(function(){
		...
		locationDatatable = $('#location-datatable').DataTable({
			...
			"info":     false,
			"searching": false,
			"ajax": "<?php echo e(route('opossum.ajax.getLocations')); ?>",
		...
		$('#merchant-location-modal').on('hidden.bs.modal', function (e) {
			...
			$.ajax({
				url:'<?php echo e(route('data.ajax.saveMerchantDefaultLocation')); ?>',
				type: 'post',
				data: {
        ...
        $('.oneWayModal').on('hidden.bs.modal', function (e) {
            ...
            if (data.company_name != ''
                ...
                $.ajax({
                    type:"POST",
                    url:'<?php echo e(route('data.ajax.saveMerchantOneway')); ?>',
                    dataType:"JSON",
                    data: data,
                    success: function(response) {
                        if(response.status == 'true') {
    ...
    $('#merchant_add_button').click(function(){
        ...
        $.ajax({
            type:'GET',
            url:'<?php echo e(route('data.ajax.saveMerchantTwoWayLinking')); ?>',
            datatype: 'json',
            data:{merchant_id:id},
            success: function(response){
...
<?php /**PATH E:\ocosystem\resources\views/data/databuttons.blade.php ENDPATH**/ ?>