...
class OposPetrolStationPumpController extends Controller
{
    ...
    public function pumpSetPrices($pumpNo, $prices, $ipaddr) {
		$conn_fail = false;
        ...
        $pts2 = new PTS2(env('PTS_USER'), env('PTS_PASSWD'), $url);
        $res = $pts2->pump_set_prices($pumpNo, $prices);
    ...
    public function pumpAuthorize($pumpNo,$type,$dose,$ipaddr,
		...
		Log::debug('pumpAuthorize:'.$pumpNo.', type='.$type.', dose='.$dose);
    ...
    public function pumpCloseTransaction($pumpNo, $transaction) {
        $pts2 = new PTS2(env('PTS_USER'), env('PTS_PASSWD'));
        $res = $pts2->pump_close_transaction($pumpNo, $transaction);
    ...
    public function setPumpsConfiguration(Request $request) {
		...
		//Log::debug('setPumpsConfiguration:'.json_encode($all));
	...
	public function pumpProductByNozzle($pump_no, $nozzle_no) {
		Log::debug('***** PS pumpProductByNozzle('.$pump_no.','.
			$nozzle_no.') *****');
		...
		if (!empty($result2)) {
			...
			$qty = 	(float) number_format( app('App\Http\Controllers\InventoryController')->
				location_productqty($result2[0]->product_id, $result2[0]->location_id),2);

			$price = (float) number_format( app('App\Http\Controllers\IndustryOilGasController')->
				getPrice($result2[0]->ogFuel_id, $result2[0]->location_id),2);
 
			
			Log::debug('MA2 pname='.json_encode($pname));
		}
