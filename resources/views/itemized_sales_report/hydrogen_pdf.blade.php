...
<body>
    ...
    <table border="0" cellpadding="0" cellspacing="0" class="table" id="item" style="margin-top: 5px; width:100%">
       ...
       @foreach($get_receipts as $key => $get_receipt)
            <tr class="table-td">
                ...
                <td class="text-center" style="border-style: none">
                    {{ number_format(($get_receipt->price/100) , 2) }}
                </td>
                <td style="text-align:center;border-style: none">
					{{ number_format($get_receipt->quantity , 2) }}
                </td>

                <td style="text-align:center;border-style: none">
                    {{  number_format(($get_receipt->sst/100) , 2) }}
                </td>
                 <td style="text-align:center;border-style: none">
                    {{ number_format($get_receipt->total , 2) }}
                </td>
                 <td style="text-align:center;border-style: none">
                    {{  number_format(($get_receipt->item_amount/100) , 2) }}
                </td>
                 <td style="text-align:center;border-style: none">
                 	@if ($get_receipt->payment_type == "creditcard")
                    {{ "Credit Card" }}
                    @else
                    {{ ucwords($get_receipt->payment_type) }}
                    @endif
