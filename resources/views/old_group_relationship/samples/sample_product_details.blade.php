			...
			<div class="col-md-6">
				...
				@foreach ($product_category as $c)
					<option class="form-control" value="{{$c->id}}"
						@if (!empty($selected_product->category_id))
						@if ( $selected_product->category_id == $c->id) selected
						@endif
						@endif>{{$c->name}}
					</option>
				@endforeach
				...
				@if (!empty($product_details->prdcategory_id))
					@foreach ($product_subcategory as $c)
						<option class="form-control" value="{{$c->id}}"
							@if (!empty($selected_product->subcategory_id))
							@if ( $selected_product->subcategory_id == $c->id) selected
							@endif
							@endif>{{$c->name}}
						</option>
					@endforeach
				...
				@if (!empty($product_details->prdcategory_id))
					@foreach ($product_product as $c)
						<option class="form-control" value="{{$c->id}}"
							@if (!empty($selected_product->prdprdcategory_id))
							@if ( $selected_product->prdprdcategory_id == $c->id) selected
							@endif
							@endif>{{$c->name}}
						</option>
					@endforeach
