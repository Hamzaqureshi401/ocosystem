	...
	<div class="d-flex" style="width: 100%">
		...
		<div class="col-sm-4 d-flex">
			...
			<div class="col-sm-8 align-self-center"
				...
				<ul style="padding-left:0;margin-bottom:0; list-style:none">
					<li><h4 style="margin-bottom:0">{{$location->branch}}</h4></li>
		...
		<div style="padding-right:0" class="col-sm-3">
			...
			<button class="btn btn-success sellerbutton" 
				...
				style="float: right; margin: 0px 0px 0;">
				Pump	
			</button>
	
			<button class="btn btn-success bg-save sellerbutton" 
				onclick="trigger_push()"
				style="float: right; margin: 0px 5px 5px;">
				Push
			</button>
		...
		<table id="tableFuelLocalPrice" class="table table-bordered">
			<thead class="thead-dark">
				<tr>
					...
					<th style="text-align:center;">Price/&ell;</th>
					<th class="text-center" style="min-width:300px;" >User</th>
				<th class="text-center" style="min-width:100px;">User Date</th>
				</tr>
			</thead>
			<tbody id="shows">
			</tbody>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
					<input class="form-control input-30" 
						placeholder="Price" id="payment_input_1"
						style="margin-left: 2px;width: 100%;display: inline-flex;text-align:right;" />
...
<div class="modal-dialog modal-dialog-centered modal-md  mw-75 w-50"
	...
	<script>
	 var tableFuelLocalPrice_table = $('#tableFuelLocalPrice').DataTable({
		...
		"order": [],
		"columnDefs": [  
			{"className": "dt-center vt_middle slimcell", "targets": [0,2,3]},
			{"className": "dt-left vt_middle slimcell", "targets": [1]},
		...
		var table_pump_popup_table = $('#pump_popup_table')
			...
			"order": [],
			"columnDefs": [  
				{"className": "dt-center vt_middle slimcell", "targets": [0,1]},
		...
		function trigger_push(){
			$.post("{{route('fuel_local_price_pump_detail_push')}}", {
				location_id: "{{$location->id}}"
			}).done( res =>{ 
				$('#res').html(res);
				$("#pumpConfiguration_").modal('show');
	...
	function sendPayload() {
		$.post('{{route('send_payload_custom')}}', {
			location_id: '{{$location->id}}',
			type: 'product'
		});
	}
