	...
	<div class="modal-content">
		...
		<form  class="m-form  m-form--state m-form--label-align-right " >
			<div class="modal-body">
                ...
                <textarea id="modal-item_remark" name="remark" class="form-control m-input" rows="3"></textarea>
				</div>
			</div>
		</form>
	</div>
...
<div class="row"
	...
	<div class="col-sm-3" style="align-self:center">
		<h2 style="margin-bottom:8px;padding-top: 0;">Product Ledger</h2>
	</div>
	...
	<table class="table table-bordered" id="ogProductLeger" style="width: 100%;">
		<thead class="thead-dark">
          <tr>
              ...
              <th class="text-center" style="width: 18%;">Location</th>
        ...
        @foreach($data as $value)
            @php
				if($value->table_name == 'stockreportproduct'){
                    ...
                    $remarks = $value->stock_report->remark->remarks ?? 'Remarks';
                    $remark_type = 'stock';
            ...
            <tr>
                ...
                <td style="text-align: center;@if($value->document_no == 1) background-color: red; @endif">
                    @if($value->table_name == 'stockreportproduct')
                ...
                <td style="text-align: center;" nowrap>
                    @if($value->table_name == 'stockreportproduct')
                        @if($value->stock_report->type)
                            @php $type = $value->stock_report->type; @endphp
                            @if($str->is('stock*', $type))
                                {{'Stock'.' '.$str->ucfirst(explode('stock',$type)[1])}}
                            @else
                                {{$str->ucfirst($type)}}
                            @endif
                        @endif
